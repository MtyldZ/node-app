...
export function sign<T extends object>(payload: T, signAs: 'user' | 'admin', expiresIn = '2w') {
    return jwt.sign(payload, signAs === 'admin' ? process.env.JWT_ADMIN_SECRET : process.env.JWT_USER_SECRET, {expiresIn});
}

export function verify<T extends object>(token: string, verifyAs: 'user' | 'admin') {
